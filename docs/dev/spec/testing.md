...
### 1.4 包形状防回归（判据：#11/#12 定案，#37 扩为 v1 hooks 形状，#46 扩为 chat.params 清闸门）
...
- chat.params 清闸门（#46）：hook 对本 provider 一律置 `output.maxOutputTokens = undefined`（照抄官方 codex.ts 模式），他 provider 不过界；清除值喂给信封的缺省端到端（缺省 `max_tokens` 落 64000）在 bun test 内。hook 的宿主触发时序仍归 §4 真宿主边界。
...
- **v1/v2 glue**：三合一入口的 `server()` / config / auth hooks、v2 `catalog.transform` 自指注册、integration 认证、`/connect` TUI——**不入 bun test**。mock 整个宿主等于重写宿主，成本远超收益；该路径已由 #5/#11/#12 三次真机实测定案，回归防线 = #21 人工验收。**glue 的宿主交互行为是全项目唯一无自动化测试的模块面**（老板知情拍板）；模块内纯函数与包形状断言按 §1.4 在 bun test 内（#37 起；#46 的 chat.params 清闸门纯行为同理）。
