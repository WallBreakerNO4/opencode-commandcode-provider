...
describe("包形状防回归", () => {
  ...
  test("v1 server() 返回 {config, auth, chat.params} hooks：auth 指向 commandcode-go、label 固定「Command Code API Key」", async () => {
    const hooks = await plugin.server({}, {})
    expect(Object.keys(hooks).sort()).toEqual(["auth", "chat.params", "config"])
